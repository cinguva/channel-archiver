...
class ChannelReaper(object):
  def __init__(self):
    ...
    self.days_inactive      = int(os.getenv('DAYS_INACTIVE', 180))
    ...
    # You can declare as environment variable or hard code DRY_RUN, token value
    self.dry_run            = (os.getenv('DRY_RUN', 'true') == 'true')
    #self.slack_token        = os.getenv('SLACK_TOKEN')
    self.slack_token = 'xoxp-*************************************'
  ...
  def get_channel_alerts(self):
    # Reads template file from the folder
  ...
  def archive_channel(self, channel, alert):
    ...
    # Append stdout_message to log file
    handler = logging.FileHandler('C:\\scripts\\Slack\\slack-autoarchiver\\audit.log')
